...
![보고서의 단계별 출력 시뮬레이션](figures/r02-p05-1.png)

`실험3 프로젝트_C135001 강가영 (1).pdf` 5페이지는 1단 gain 3.172, 2단 16.24, 전체 51.53을 기록합니다. 위 51.0은 별도 캡처에서 계산한 값이므로 같은 실행의 동일 값으로 합치지 않습니다. 제공된 파형은 시뮬레이터 결과이며 실제 계측기 실측으로 표시하지 않습니다.
