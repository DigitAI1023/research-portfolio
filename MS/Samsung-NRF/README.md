...
개발 중 기록입니다. tap ratio가 1인 경우의 FIFO indexing 문제를 명시적 read/write pointer로 수정하고, 40 sample에서 앞쪽 CP 8 sample을 제거해 32 sample이 출력되는지 확인했습니다.

![Shift 설정값 적용](figures/np1-s29-0.png)

Configuration address 폭이 바뀌면 shift 값 위치가 달라지는 문제도 있었습니다. 고정 비트 위치를 주소 폭에 따른 파라미터로 바꾼 뒤 설정 적용을 확인했습니다. 위 그림은 개발 중 RTL 검증이며 앞의 ZCU208 실측 결과와 구분합니다.
